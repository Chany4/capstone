...
export default createStore({
  ...
  actions: {
    ...
    async updateMech(context, id) {
      console.log(id);
      try {
        const response = await (await axios.patch(`${apiURL}bigTime/mech/',id`)).data
        Swal.fire({
          title: "Good job!",
    ...
    async loginUser({
      ...
      try {
        ...
        let {
          ...
          token,
          user
        } = await (await axios.post(`${apiURL}bigTime/login`, currentUser)).data
        console.log({
          message,
          token,
          user
